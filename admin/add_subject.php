  ...
  <div class="">
    ...
    <section class="ab-info-main py-md-5 py-4">
        <div class="container py-md-5 py-4">
            <h3 class="tittle text-center mb-lg-5 mb-3"><span class="sub-tittle">Add</span>Subject here</h3>
              ...
              <div class="col-lg-5">
                <form method="post">
                  <div class="form-group">
                    ...
                    <input class="form-control" type="text" name="subject_id" placeholder="Enter Subject Id" required>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    <select class="form-control" name="department_name_s_id" required>
                  ...
                  <button type="submit" name="add_subject_button" class="btn btn-primary submit mb-4">ADD</button>
...
<?php
    ...
    if (isset($_POST['subject_id'],$_POST['subject_name'],$_POST['department_name_s_id'],$_POST['semester'])) {
        $s_id=strtolower(mysqli_real_escape_string($conn, $_POST["subject_id"]));
        $s_name=strtolower($_POST["subject_name"]);
        $d_name_id=$_POST["department_name_s_id"];
        $sem=$_POST["semester"];

        // validation
        if (empty($s_id) || empty($s_name) || empty($d_name_id) || empty($sem)) {
            header('Location: add_subject.php?message=Empty_fields');
            exit();
        }
        if (!preg_match("/[a-z0-9]/i", $s_id)) {
            header('Location: add_subject.php?message=Invalid_subject_id');
            exit();
        }

        $sql = "INSERT INTO `subjects` (`subject_id`, `subject_name`, `department_id`, `semester`) VALUES
        ('$s_id', '$s_name', '$d_name_id','$sem')";
        $result=mysqli_query($conn, $sql);

        if (!$result) {
            echo '<script language="javascript">';
            echo 'alert("Invalid subject id or subject name")';
            echo '</script>';
            unset($_POST);
        } else {
            echo '<script language="javascript">';
            echo 'alert("Successfully inserted the subject")';
            echo '</script>';
            unset($_POST);
        }
    }
